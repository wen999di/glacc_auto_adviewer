...
// 禁用 GPU 硬件加速：避免启动 GPU 子进程，使 app.exit() 退出时无需等待 GPU 清理，
// 可即时返回终端。广告 WebView 通过软件渲染依然可以正常显示。
// 注意：必须在 app 准备就绪前调用。
...
function encryptAccountDat(jsonString) {
  try {
    const cipher = crypto.createCipheriv('aes-128-ecb', ACCOUNT_KEY, '');
    return Buffer.concat([cipher.update(Buffer.from(jsonString, 'utf8')), cipher.final()]);
  } catch {
    return null;
  }
}
...
function extractFromAccountFile(filePath) {
  ...
  for (const candidate of candidates) {
    ...
    // GlaccAvalonia 格式：解密后为简单 JSON，字段首字母大写
    // 结构示例: { AccessToken, RefreshToken, UserId, Nickname, DeviceId, TokenExpiry }
    if (data.AccessToken) {
      return {
        accessToken:  data.AccessToken,
        refreshToken: data.RefreshToken || null,
        tokenExpiry:  data.TokenExpiry  || null,  // ISO 日期字符串，如 "2025-01-01T00:00:00"
        userId:       data.UserId       || null,
        sourceFile:   candidate,
        rawData:      data,  // 完整原始数据，用于刷新后写回时保留 Nickname、DeviceId 等字段
      };
    }
...
/**
 * 使用 TokenExpiry 字段判断 token 是否已过期（比 JWT exp 字段更可靠）。
 * 对齐 GlaccAvalonia 的 IsAccessTokenValid：null 表示不检查，提前 60s 视为过期。
 * @param {string|null} tokenExpiry  ISO 日期字符串
 * @returns {boolean}
 */
function isTokenExpiredByExpiry(tokenExpiry) {
  if (!tokenExpiry) return false;
  const expiry = new Date(tokenExpiry);
  if (isNaN(expiry.getTime())) return false;
  return expiry <= new Date(Date.now() + 60 * 1000);
}

/**
 * 使用 refresh_token 请求新的 access_token。
 * POST https://user.geilijiasu.net/v1/auth/token
 * @param {string} refreshToken
 * @returns {Promise<{accessToken:string, expiresIn:number}>}
 */
function refreshAccessToken(refreshToken) {
  const CLIENT_ID = 'aF42mN5OP0hUa0nK';
  const refreshUrl = `${XBASE_ORIGIN}/v1/auth/token`;
  const CLIENT_SECRET = '4TgNf1MdEfC0KeZoFAJ3vQ';
  ...
  const urlParsed = new URL(refreshUrl);
      ...
      res.on('end', () => {
        try {
          ...
          if (json.access_token) {
            resolve({ accessToken: json.access_token, expiresIn: json.expires_in || 0 });
          } else {
            reject(new Error(`Token refresh failed (HTTP ${res.statusCode}): ${buf}`));
          }
        } catch (e) {
          reject(e);
        }
...
let REFRESH_TOKEN = null;  // 来自 GlaccAvalonia 格式账户文件
let TOKEN_PROBE_FAILED = false;  // 启动时 probe 检测到 token 无效
const AUTO_REFRESH_TOKEN = args.includes('--autorefreshtoken');

// 持久化账号文件路径和原始数据（供运行时 token 刷新使用）
let _accountFilePath = null;
let _accountRawData = null;
...
async function postAdStatsCompat(statsData, taskId = 0) {
  ...
  try {
    ...
    for (let attempt = 1; attempt <= 3; attempt += 1) {
      ...
      // 认证失败（token 过期）：无需重试，立即中止并通知 renderer。
      const s = Number(res?.status);
      const bodyCode = Number(res?.data?.code);
      if (s === 401 || s === 403 || bodyCode === 16 || bodyCode === 4052) {
        try { mainWindow?.webContents?.send('ad:authExpired'); } catch { /* ignore */ }
        break;
      }
...
async function handleTransferFetch(requests, context = {}) {
  ...
  while (true) {
    ...
    // 对齐 original：一方域名 API 返回 UNAUTHENTICATED（HTTP 401/403 或 code 16/4052）时，
    // 说明 token 已过期/失效。通知 renderer，自动模式下立即退出，避免永久卡住。
    if (isFirstParty) {
      const bodyCode = resp?.data?.code;
      if (status === 401 || status === 403 || bodyCode === 16 || bodyCode === 4052) {
        try { mainWindow?.webContents?.send('ad:authExpired'); } catch { /* ignore */ }
      }
    }
...
// ─── 本地检测 JWT token 是否已过期（仅供调试参考，不用于业务逻辑）────────────────────
// 注意：access_token 的实际服务端过期时间与 JWT exp 字段可能不一致，
// 因此不应依赖此函数作为过期判断依据；应以实际 API 响应（401/403/code16）为准。
function isTokenExpired(token) {
  if (!token) return true;
  try {
    const payload = token.split('.')[1];
    const decoded = JSON.parse(Buffer.from(payload, 'base64url').toString());
    if (typeof decoded.exp !== 'number') return false; // 无 exp 字段，无法判断
...
ipcMain.handle('ad:init', () => {
  ...
  return {
    ...
    timeout: timeout,
    tokenInvalid: TOKEN_PROBE_FAILED,
  };
});
...
ipcMain.handle('ad:getConfig', async () => {
  ...
  // 注意：不通过 JWT exp 字段判断过期（实际过期时间与 JWT 标注可能不一致），
  // 只根据服务端实际响应（401/403/code 16/4052）判断。
  try {
    const resp = await getAdConfig();
    // 服务端以 401/403 或 body code 16（UNAUTHENTICATED）/ 4052（expired_token）表示 token 失效
    if (resp.status === 401 || resp.status === 403) return { error: 'token_expired' };
    const bodyCode = resp?.data?.code;
    if (bodyCode === 16 || bodyCode === 4052) return { error: 'token_expired' };
    return resp;
  } catch (e) {
...
ipcMain.handle('ad:getWallet', async () => {
  ...
  // 只根据服务端实际响应（401/403/code 16/4052）判断。
  try {
    const resp = await makeRequest(USER_WALLET_URL, 'GET', {});
    if (resp.status === 401 || resp.status === 403) return { error: 'token_expired' };
    const bodyCode = resp?.data?.code;
    if (bodyCode === 16 || bodyCode === 4052) return { error: 'token_expired' };
    return resp;
...
ipcMain.on('ad:exit', async (event, message) => {
  // 使用 app.exit() 而非 process.exit()：Electron 中 app.exit 更可靠，
  // 可确保整个 Electron 进程（含所有子窗口）立即退出，不依赖 Node.js 事件循环。
  const msg = message || '';
  // token 过期时：若 --autorefreshtoken 且有 refresh_token，先尝试刷新并写回文件，再退出
  if (msg === 'error:token_expired') {
    await tryRuntimeTokenRefresh();
  }
...
ipcMain.handle('ad:getADCompletionInfo', async () => {
  try {
    ...
    // 检测鉴权失败：通知宣宿页面以便 auto 模式能及时退出
    const bodyCode = resp?.data?.code;
    if (resp.status === 401 || resp.status === 403 || bodyCode === 16 || bodyCode === 4052) {
      try {
        if (mainWindow && !mainWindow.isDestroyed()) {
          mainWindow.webContents.send('ad:authExpired');
        }
      } catch {}
    }
    return resp;
...
/**
 * 运行时 token 刷新：仅在 --autorefreshtoken 模式下且有 refresh_token 时尝试。
 * 成功后更新 ACCESS_TOKEN 并将新 token 写回账号文件（含 TokenExpiry）。
 * @returns {Promise<boolean>} 是否刷新成功
 */
async function tryRuntimeTokenRefresh() {
  if (!AUTO_REFRESH_TOKEN || !REFRESH_TOKEN || !_accountFilePath || !_accountRawData) return false;
  try {
    const refreshed = await refreshAccessToken(REFRESH_TOKEN);
    ACCESS_TOKEN = refreshed.accessToken;
    currentUserId = getUserIdFromToken(ACCESS_TOKEN) || currentUserId;
    const newExpiry = (refreshed.expiresIn && refreshed.expiresIn > 0)
    ...
    const encrypted = encryptAccountDat(JSON.stringify(updatedData));
    if (encrypted) {
      fs.writeFileSync(_accountFilePath, encrypted);
      _accountRawData = updatedData;
    }
    return true;
  } catch (e) {
    console.error('[ad-viewer] Runtime token refresh failed:', e.message);
    return false;
  }
}
...
app.whenReady().then(async () => {
  ...
  if (ACCOUNT_FILE_ARG !== undefined) {
    ...
    let accountRawData = null;
    if (creds && creds.accessToken) {
      ACCESS_TOKEN = creds.accessToken;
      currentUserId = getUserIdFromToken(ACCESS_TOKEN);
      accountFilePath = creds.sourceFile || null;
      accountRawData  = creds.rawData   || null;
    } else {
    ...
    if (creds && creds.refreshToken) REFRESH_TOKEN = creds.refreshToken;

    // 将账号文件路径和原始数据提升到模块级全局，供运行时刷新使用
    _accountFilePath = accountFilePath;
    _accountRawData  = accountRawData;

    // 通过实际 API 请求探测 access_token 是否有效
    // 使用 /v1/user/me（原版 app 的 USER_ME_URL），比依赖本地时间戳更可靠
    if (ACCESS_TOKEN) {
      try {
        const probe = await makeRequest(`${XBASE_ORIGIN}/v1/user/me`, 'GET', {});
        const probeCode = probe?.data?.code;
        // HTTP 4xx/5xx 或业务码非零均视为 token 无效
        const probeFailed = (probe.status >= 400) || (probeCode !== undefined && probeCode !== 0 && probeCode !== null);
        if (probeFailed) {
          process.stdout.write(`[probe] token invalid (HTTP ${probe.status}, code=${probeCode ?? 'n/a'})\n`);
          if (AUTO_REFRESH_TOKEN && REFRESH_TOKEN && accountFilePath && accountRawData) {
            // --autorefreshtoken 模式：尝试用 refresh_token 获取新 token
            process.stdout.write('[probe] attempting token refresh via refresh_token...\n');
            try {
              const refreshed = await refreshAccessToken(REFRESH_TOKEN);
              ACCESS_TOKEN = refreshed.accessToken;
              currentUserId = getUserIdFromToken(ACCESS_TOKEN) || currentUserId;
              // 将更新后的 AccessToken 和 TokenExpiry 写回账号文件（保留其他字段如 Nickname、DeviceId）
              const newExpiry = (refreshed.expiresIn && refreshed.expiresIn > 0)
                ? new Date(Date.now() + refreshed.expiresIn * 1000).toISOString()
                : null;
              const updatedData = {
              ...
              const encrypted = encryptAccountDat(JSON.stringify(updatedData));
              if (encrypted) {
                fs.writeFileSync(accountFilePath, encrypted);
                _accountRawData = updatedData;  // 同步更新全局，供运行时刷新使用
                process.stdout.write('[probe] token refreshed and written back successfully\n');
              } else {
                process.stdout.write('[probe] WARNING: token refreshed but encryptAccountDat returned null, file not updated\n');
              }
            } catch (e) {
              process.stdout.write(`[probe] token refresh failed: ${e.message}\n`);
              TOKEN_PROBE_FAILED = true;
              ACCESS_TOKEN = null;
            }
          } else if (AUTO_REFRESH_TOKEN && !REFRESH_TOKEN) {
            process.stdout.write('[probe] --autorefreshtoken set but no RefreshToken found in account file\n');
            TOKEN_PROBE_FAILED = true;
            ACCESS_TOKEN = null;
          } else {
            TOKEN_PROBE_FAILED = true;
        ...
        // 网络错误（无法连接服务器）不视为 token 过期，继续正常流程
      }
    }
  }
